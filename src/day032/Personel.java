...
public class Personel {
  ...
  /*  public static void maasaZamYap(Personel personel,int zamOrani){
        if (personel==null | zamOrani<=0) return; //   || yani short circuitte ilk kısım doğruysa zamOranina hiç bakmaz
            //2. kısmın null ya da yanlış olup programı kırması ihtimaline karşın 2.ye baktırmıyor.
    ...
    public static boolean maasaZamYap(Personel personel,int zamOrani){
        if (personel==null | zamOrani<=0) return false;
         personel.setMaasi(personel.getMaasi()*(100+zamOrani)/100); //Böyle de yapabiliriz.
        return true;
